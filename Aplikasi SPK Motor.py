...
st.set_page_config(page_title="SPK Motor - SAW", layout="centered")
...
with st.form("input_form"):
    harga_max = st.number_input("1. Berapa harga maksimal motor yang Anda inginkan? (dalam Rp ribu)", 
                                min_value=10000, max_value=50000, value=30000, step=1000)
    min_konsumsi = st.number_input("2. Berapa konsumsi bahan bakar minimal yang Anda inginkan? (km/liter)", 
                                   min_value=30, max_value=70, value=45, step=1)
    min_kapasitas = st.number_input("3. Berapa kapasitas mesin minimal yang Anda inginkan? (cc)", 
                                    min_value=100, max_value=200, value=125, step=5)
    desain = st.slider("4. Seberapa penting desain/tampilan bagi Anda?", 1, 5, 3)
    tanki_min = st.number_input("5. Berapa kapasitas tangki minimal yang Anda inginkan? (liter)", 
                                min_value=3.0, max_value=7.0, value=4.0, step=0.1)
...
if submitted:
    ...
    data = {
        ...
        'C1': [26000, 32000, 24000, 19000, 29000],       # Harga (Rp ribu)
        'C2': [46, 40, 54, 60, 45],                      # Konsumsi (km/l)
        'C3': [160, 155, 125, 110, 155],                 # Kapasitas Mesin (cc)
        'C4': [4, 5, 3, 4, 5],                           # Desain (1-5)
        'C5': [5.5, 7.1, 5.2, 4.0, 4.5],                 # Tangki (L)
        'C6': [5, 5, 4, 3, 4]                            # Fitur keselamatan (1-5)
    }
